...
with col_model:
    selected_display_name = st.selectbox("Select Model", model_names)
    metrics = models[selected_display_name]
    ...
    mcol2.markdown(f"<span style='font-size:1rem;'>F1 Score</span><br><span style='font-size:1.5rem;font-weight:bold'>{metrics['f1-score']:.2%}</span>", unsafe_allow_html=True)
with col_upload:
    ...
    if predict_clicked:
        with st.spinner("Predicting..."):
            try:
                ...
                data = {"model_name": selected_display_name}
